...
export function ScenarioSimulator({ materialId, histories }: { materialId: number; histories: MaterialDriverHistory[] }) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-busy={loading}
            className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
            {loading && (
              <svg
      ...
      {loading && (
        <div className="mt-5 flex flex-col items-center justify-center rounded-md border border-slate-200 bg-white p-8 text-center" aria-live="polite">
          <svg
            className="h-7 w-7 animate-spin text-teal-700"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            aria-hidden="true"
          ...
          <p className="mt-3 text-sm font-medium text-slate-800">Simulating scenario forecast...</p>
          <p className="mt-1 text-xs text-slate-500">Recalculating price trajectory and recommendation based on driver adjustments.</p>
        </div>
      )}
      {!loading && comparison && (
